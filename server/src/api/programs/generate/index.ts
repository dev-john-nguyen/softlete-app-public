...
router.post('/', async (req: any, res: any, next: any) => {
    ...
    if (!startDate || typeof startDate !== 'string') return res.status(401).send("Invalid date provided.")
    ...
    if (!dObj || Object.prototype.toString.call(dObj) !== '[object Date]') return res.status(401).send("Failed to convert date.")
    ...
    if (!program) return res.status(404).send("Program not found. Please try again.");
    ...
    //get all workouts
    const programWorkouts = await ProgramTemplateWorkouts.find({ programTemplateUid: programData._id }).select(filter);

    if (!programWorkouts || programWorkouts.length < 1) return res.status(401).send("Couldn't found any workouts associated with this program. Please try again")

    //get all exercises associated with this program
    const programExercises = await ProgramTemplateExercises.find({ programTemplateUid: programData._id }).select({ ...filter, _id: 0 });

    //get all health data
    const programHealthData = await programTemplateHealthData.find({ programTemplateUid: programData._id }).select({ ...filter, _id: 0 });

    const { _id: programTemplateUid, userUid: programUserUid, ...rest } = programData;
    ...
    const newWorkouts = programWorkouts.map(workout => {
        const { daysFromStart, _id: workoutUid, ...workoutRest } = workout.toObject();
        ...
        const exercises: WorkoutExercisesProps[] = programExercises.filter(e => e.toObject().programWorkoutUid.equals(workoutUid)).map(e => {
            ...
            const calcRef = uid === programData.userUid && e.calcRef ? e.calcRef : 0;
            return {
                ...eRest,
                workoutUid: newId,
                userUid: uid,
        ...
        const healthData = programHealthData.find(data => data.programWorkoutUid.equals(workoutUid))
    ...
    if (newWorkouts.length < 1) {
        ...
        return res.status(401).send("Need to have at least one workout in the program.")
    }

    let insertedWos: (WorkoutProps & mongoose.Document<any, any, WorkoutProps>)[] = [];
    ...
    let insertedExs: (WorkoutExercisesProps & mongoose.Document<any, any, WorkoutExercisesProps>)[] = [];
    ...
    try {
        ...
        return res.status(500).send("An error occurred when trying to inset exercises.");
    }

    //insert health data
    let insertedHealthData: (WorkoutHealthDataProps & mongoose.Document<any, any, WorkoutHealthDataProps>)[] = [];
    ...
    try {
        ...
        return res.status(500).send("An error occurred when trying to insert health data.");
    }
    ...
    if (programUserUid !== uid) {
        ...
        if (exUids.length > 0) {
            ...
            try {
                ...
                //find difference
                const newUserMeasDocs = _.differenceBy(programUserMeasDocs, userMeasDocs, 'exerciseUid');
    ...
    handleGenProgramNotification(programUserUid, uid, newProgram.name)
