...
import ProgramTemplateHealthData, { ProgramTemplateHealthDataProps } from '../../../collections/program-template-health-data';
...
router.post('/', async (req: any, res: any, next: any) => {
    ...
    if (!mongoose.Types.ObjectId.isValid(workoutUid)) return res.status(401).send("Invalid workout id.")

    if (typeof daysFromStart !== 'number') return res.status(401).send("Invalid data.")

    let duplicateWorkout: ProgramTemplateWorkoutsProps & import("mongoose").Document<any, any, ProgramTemplateWorkoutsProps>;
    let workout: any;
    let exercises = [];
    ...
    try {
        ...
        workout = await ProgramWorkouts.findById(workoutUid).select({
            name: 1, programTemplateUid: 1, description: 1, _id: 0, userUid: 1, type: 1
        });

        if (!workout) return res.status(404).send('Workout was not found.');

        exercises = await ProgramExercises.find({ programWorkoutUid: workoutUid }).select({
            _id: 0,
            createdAt: 0,
            updatedAt: 0,
            _v: 0,
    ...
    let savedExercises: (ProgramTemplateExercisesProps & mongoose.Document<any, any, ProgramTemplateExercisesProps>)[] = []
    ...
    try {
        ...
        const workoutHealthData = await ProgramTemplateHealthData.findOne({ programWorkoutUid: workoutUid, userUid: workout.userUid }).select({
            _id: 0,
            createdAt: 0,
            updatedAt: 0,
            _v: 0,
