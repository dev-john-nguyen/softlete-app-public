...
import WorkoutExercises, { WorkoutExerciseDataProps } from '../../../collections/workout-exercises';
...
router.post('/', (req: any, res: any, next: any) => {
    ...
    for (let i = 0; i < dataArr.length; i++) {
        ...
        if (isInvalidExerciseData(data)) return res.status(400).send("Invalid data associated with request");
    ...
    WorkoutExercises.bulkWrite(updateBatch)
        .then((batch) => {
