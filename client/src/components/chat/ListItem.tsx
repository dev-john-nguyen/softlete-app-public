...
import Animated, { useAnimatedStyle, useAnimatedGestureHandler, useSharedValue, withTiming, runOnJS } from 'react-native-reanimated';
...
const ChatListItem = ({ chat, user, onChatPress, onRemoveChat }: Props) => {
    ...
    const userProps: AthleteProfileProps | undefined = chat.usersProps?.find(u => u.uid !== user.uid)
    const unread = chat.recentUser && chat.recentUser !== user.uid && !chat.read ? true : false;
            ...
            <Animated.View style={animatedStyle}>
                <PanGestureHandler onGestureEvent={gestureHandler}>
                    <Animated.View style={styles.content}>
                        ...
                        <View style={[styles.unread, { backgroundColor: unread ? BaseColors.primary : BaseColors.white }]} />

                        <View style={styles.image}>
                            <ProfileImage imageUri={getImageUri()} />
                        </View>
                        <View style={{ marginLeft: StyleConstants.smallMargin, flex: 1 }}>
                            <SecondaryText styles={styles.username} bold>{userProps ? userProps.username : 'unknown'}</SecondaryText>
                            <SecondaryText styles={styles.message} numberOfLines={1}>{chat.recentMsg ? chat.recentMsg : ''}</SecondaryText>
                        </View>
                        <View style={styles.timeContainer}>
                            <SecondaryText styles={styles.time} bold>{getTimeAgo(chat.recentTime)}</SecondaryText>
                        </View>
                    </Animated.View>
                </PanGestureHandler>
            </Animated.View>
