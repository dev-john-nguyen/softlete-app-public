...
import { HealthDataProps, HealthDisMeas } from '../../../../services/workout/types';
...
import { convertMsToTime } from '../helpers/format';
                ...
                case 'duration':
                    return (
                        <View style={styles.editContainer}>
                            <View style={{ flex: 1 }}>
                                {this.renderBack(() => this.setState({ editName: '' }))}
                                <DurationForm onDurationUpdate={(num) => this.setState({ duration: num })} />
                            </View>
                            <HealthItem svg={<ClockSvg fillColor={BaseColors.primary} />} label='Duration' text={convertMsToTime(this.state.duration)} edit />
                        </View>
                    )
                case 'distance':
                    return (
                        <View style={styles.editContainer}>
                            <View style={{ flex: 1 }}>
                                ...
                                <MeasForm
                                    ...
                                    onChangeDistance={(txt) => this.setState({ distance: this.onParseText(txt) })}
                                />
                            </View>
                            <HealthItem svg={<RulerSvg fillColor={BaseColors.primary} />} label='Distance' text={`${this.state.distance} ${this.state.disMeas}`} edit />
                        </View>
                    )
                case 'calories':
                    return (
                        <View style={styles.editContainer}>
                            <View style={{ flex: 1 }}>
                                ...
                                <BaseForm
                                    ...
                                    onChange={(txt) => this.setState({ calories: this.onParseText(txt) })}
                                    placeholder={this.renderPlaceHolder(this.state.calories)}
                                    title={'Calories'}
                                    label={'kcal'}
                                />
                            </View>
                            <HealthItem svg={<FireSvg fillColor={BaseColors.primary} />} label='Calories' text={`${this.state.calories} kcal`} edit />
                        </View>
                    )
                case 'avghr':
                    return (
                        <View style={styles.editContainer}>
                            <View style={{ flex: 1 }}>
                                ...
                                <BaseForm
                                    ...
                                    onChange={(txt) => this.setState({ avgHr: this.onParseText(txt, true) })}
                                    placeholder={this.renderPlaceHolder(this.state.avgHr)}
                                    title={'Avg Heart Rate'}
                                    label={'bpm'}
                                />
                            </View>
                            <HealthItem svg={<HeartSvg fillColor={BaseColors.primary} />} label='Avg HR' text={`${this.state.avgHr} bpm`} edit />
                        </View>
                    )
            }
        }
        ...
        return (
            <View style={styles.container}>
                ...
                <ScrollView contentContainerStyle={{ alignItems: 'flex-start' }} horizontal>
                    <HealthItem svg={<ClockSvg fillColor={BaseColors.primary} />} label='Duration' text={convertMsToTime(this.state.duration)} onPress={() => this.setState({ editName: 'duration' })} edit />
                    <HealthItem svg={<RulerSvg fillColor={BaseColors.primary} />} label='Distance' text={`${this.state.distance} ${this.state.disMeas}`} onPress={() => this.setState({ editName: 'distance' })} edit />
                    <HealthItem svg={<FireSvg fillColor={BaseColors.primary} />} label='Calories' text={`${this.state.calories} kcal`} onPress={() => this.setState({ editName: 'calories' })} edit />
                    <HealthItem svg={<HeartSvg fillColor={BaseColors.primary} />} label='Avg HR' text={`${this.state.avgHr} bpm`} onPress={() => this.setState({ editName: 'avghr' })} edit />
                </ScrollView>
            </View>
        )
    }
