...
const DayFilter = ({ curDay, onChangeCurDay, onLongPress, groupByDay }: Props) => {
    ...
    const pressableStyle = (pressed: boolean, index: number) => {
        return [styles.itemContainer, {
            backgroundColor: pressed ? BaseColors.lightGrey : 'transparent',
            borderBottomColor: curDay === index ? BaseColors.primary : 'transparent',
        }]
    }
    ...
    const renderItem = useCallback(({ item, index }: { item: string, index: number }) => (
        ...
        >
            <SecondaryText
                styles={[styles.text, {
                    color: curDay === index ? BaseColors.primary : BaseColors.secondary
                }]}
                bold
            >
                {item}
