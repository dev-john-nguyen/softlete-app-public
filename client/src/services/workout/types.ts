import { HealthActivity, HKWorkoutQueriedSampleType } from "react-native-health";
...
export const WorkoutTypes = {
    TraditionalStrengthTraining: AppleHealthKit.Constants.Activities.TraditionalStrengthTraining,
...
interface SaveNewWorkoutExercises extends Omit<WorkoutExerciseProps, 'exercise' | 'tempId'> {
    exerciseUid: string;
}

export interface SaveEditWorkoutProps extends Omit<EditWorkoutProps, 'exercises'> {
...
export interface WorkoutActionProps {
    updateWorkoutExercises: (workoutUid: string, exercises: WorkoutExerciseProps[], removedExercises?: WorkoutExerciseProps[]) => Promise<void>;
    fetchWorkouts: (FromDate: string, toDate: string) => Promise<void | undefined>;
    ...
    updateWorkoutStatus: (workoutUid: string, status: WorkoutStatus) => Promise<void>;
    updateWorkoutExerciseData: (dataArr: DataArrProps[]) => Promise<void | WorkoutExerciseProps[]>;
    completeWorkout: (workout: WorkoutProps, strainRating: number, reflection: string, image?: ImageProps) => Promise<void>;
    updateWorkoutHeader: (workoutHeader: WorkoutHeaderProps) => Promise<void>;
    addExerciseToWorkout: (exercise: ExerciseProps, group: number, order: number) => Promise<void>;
    setViewWorkout: (workoutUid: string) => Promise<void>;
    removeWorkoutExercise: (exercise: WorkoutExerciseProps) => Promise<void>;
    updateWoHealthData: (workoutUid: string, healthData: HealthDataProps) => Promise<void>;
