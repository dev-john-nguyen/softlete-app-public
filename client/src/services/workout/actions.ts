...
import { WorkoutExerciseProps, WorkoutProps, WorkoutStatus, DataArrProps, WorkoutHeaderProps, HealthDataProps } from "./types";
import { setBanner } from "../banner/actions";
import { UPDATE_WORKOUTS, SET_SELECTED_DATE_WORKOUTS, REMOVE_WORKOUTS, SET_VIEW_WORKOUT, UPDATE_WORKOUT_INFO, UPDATE_WORKOUT_EXERCISES, INSERT_WORKOUT_FETCHED_MONTH, SET_HEALTH_DATA, INSERT_HEALTH_DATA } from "./actionTypes";
...
import { insertExercisesIntoWorkouts, insertExercises } from "../exercises/actions";
import _ from 'lodash';
import { processAnalyticsExercises } from "../misc/actions";
import { MeasSubCats, MeasCats, Categories, MuscleGroups, Equipments } from "../exercises/types";
...
export const updateWorkoutHeader = (workoutHeader: WorkoutHeaderProps) => async (dispatch: AppDispatch, getState: () => ReducerProps) => {
    let path;
    ...
    if (global.offline) {
        ...
        if (workoutHeader._id) {
            ...
            //new workout
            if (!validateWo()) {
                dispatch(setBanner(BannerTypes.warning, "You exceeded the daily limit of 5 workouts for this day."))
    ...
    return request('POST', path, dispatch, { workouts: [workoutHeader] })
        .then(({ data }: { data?: WorkoutHeaderProps[] }) => {
            if (data) {
                if (workoutHeader._id) {
...
export const updateWorkoutExercises = (workoutUid: string, exercises: WorkoutExerciseProps[], removedExercises?: WorkoutExerciseProps[]) => async (dispatch: AppDispatch, getState: () => ReducerProps) => {
    ...
    if (offline) {
        dispatch({
            type: UPDATE_WORKOUT_EXERCISES, payload: {
                _id: workoutUid,
                exercises: _.uniqBy(saveExercises, '_id').map(e => ({ ...e, tempId: AutoId.newId(20), workoutUid: workoutUid }))
            }
        })
    } else {
        const saveData = {
        ...
        const { data: resData }: { data?: WorkoutExerciseProps[] } = await request('POST', PATHS.workouts.updateExercises, dispatch, saveData)
        if (!resData) return;
        const resDataExs = await insertExercises(resData)(dispatch, getState);
        dispatch({
            type: UPDATE_WORKOUT_EXERCISES, payload: {
                _id: workoutUid,
                exercises: removedExercises ? _.uniqBy([...resDataExs, ...removedExercises], '_id') : [...resDataExs]
            }
        })
    }
...
export const removeWorkoutExercise = (exercise: WorkoutExerciseProps) => async (dispatch: AppDispatch, getState: () => ReducerProps) => {
    ...
    return request('POST', PATHS.workouts.removeExercise, dispatch, { exerciseUid: exercise._id })
...
export const fetchWorkouts = (fromDate: string, toDate: string) => async (dispatch: AppDispatch, getState: () => ReducerProps) => {
    ...
    if (global.offline) {
        const offlineWosStr = await AsyncStorage.getItem(LocalStoragePaths.offlineWos)
        if (offlineWosStr) {
            const offlineWos: WorkoutProps[] = JSON.parse(offlineWosStr);
            const workouts = await insertExercisesIntoWorkouts(offlineWos)(dispatch, getState)
    ...
    const { data }: { data?: WorkoutProps[] } = await request('GET', PATHS.workouts.fetch(fromDate, toDate, user.uid), dispatch);
    ...
    if (data) {
        if (data.length < 1) {
            ...
            const workouts = await insertExercisesIntoWorkouts(data)(dispatch, getState)
            dispatch({ type: UPDATE_WORKOUTS, payload: workouts })
            dispatch({ type: SET_SELECTED_DATE_WORKOUTS })
        }

        dispatch({ type: INSERT_WORKOUT_FETCHED_MONTH, payload: toFetchDate.getMonth() })
    }
}

export const removeWorkout = (workoutUid: string) => async (dispatch: AppDispatch, getState: () => ReducerProps) => {
    ...
    if (!global.offline) {
        ...
        request('POST', PATHS.workouts.remove, dispatch, { workoutUids: [workoutUid] })
            .then(({ data }) => {
...
export const duplicateWorkout = (date: string) => async (dispatch: AppDispatch, getState: () => ReducerProps) => {
    const { workout, global } = getState();

    if (!workout.copiedWorkout || !date) return; //empty

    if (global.offline) {
        const dupWorkout = workout.workouts.find(wo => wo._id === workout.copiedWorkout);
        if (dupWorkout) {
    ...
    const { data }: { data?: WorkoutProps } = await request('POST', PATHS.workouts.duplicate, dispatch, { workoutUid: workout.copiedWorkout, date })
    ...
    if (data) {
        ...
        dispatch({ type: UPDATE_WORKOUTS, payload: workouts ? workouts : [data] })
        dispatch(setBanner(BannerTypes.default, 'Pasted!'))
    }
}

export const setViewWorkout = (workoutUid: string, isProgram?: boolean) => async (dispatch: AppDispatch, getState: () => ReducerProps) => {
    ...
    if (viewWorkout && viewWorkout.exercises) {
        ...
        //insert exercises
        clonedWorkout.exercises = await insertExercises(clonedWorkout.exercises)(dispatch, getState)
...
export const updateWorkoutStatus = (workoutUid: string, status: WorkoutStatus, online?: boolean) => async (dispatch: AppDispatch, getState: () => ReducerProps) => {
    const { offline } = getState().global;

    if (!offline && !online) {
        const { data } = await request("POST", PATHS.workouts.updateStatus, dispatch, { _id: workoutUid, status })
        if (!data) return;
    }
...
export const updateWorkoutExerciseData = (dataArr: DataArrProps[]) => async (dispatch: AppDispatch, getState: () => ReducerProps) => {
    //validate data
    ...
    const updatedExercises: WorkoutExerciseProps[] = exercises.map(e => {
        const targetE = dataArr.find(d => (d._id && d._id === e._id) || (d.tempId && d.tempId === e.tempId));
        if (targetE) {
            e.data = targetE.data
    ...
    if (!offline) {
        const res = await request("POST", PATHS.workouts.updateData, dispatch, { dataArr })
            .catch((err) => {
                console.log(err)
            })
...
export const completeWorkout = (workout: WorkoutProps, strainRating: number, reflection: string, image?: ImageProps, online?: boolean) => async (dispatch: AppDispatch, getState: () => ReducerProps) => {
        ...
        dispatch({
            ...
            payload: {
                ...
                updatedData: {
                    ...
                    localImageUri: image ? image.uri : workout.imageUri ? workout.imageUri : ''
                }
            }
        })
        return;
    ...
    const { data }: { data?: WorkoutProps } = await request("POST", PATHS.workouts.complete, dispatch, { _id: workout._id, status: WorkoutStatus.completed, strainRating, reflection, imageId, localImageUri })
...
export const updateWoHealthData = (workoutUid: string, healthData: HealthDataProps) => async (dispatch: AppDispatch, getState: () => ReducerProps) => {

    const { global } = getState();
    let duration = healthData.duration;

    if (!duration && healthData.start && healthData.end) {
        duration = new Date(healthData.end).getTime() - new Date(healthData.start).getTime();
    }

    const healthObj: HealthDataProps = {
        workoutUid: workoutUid,
        activityId: healthData.id ? healthData.id : AutoId.newId(20),
...
export const getAllHealthData = () => async (dispatch: AppDispatch, getState: () => ReducerProps) => {
    const { user } = getState()
