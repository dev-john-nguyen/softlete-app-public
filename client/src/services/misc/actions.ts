...
import { calcAnalytics, reduceExercises, recalcAverage, calcAnalyticsData } from "./utils";
        ...
        .then(async ({ data }: { data?: WorkoutExerciseProps[] }) => {
            if (data) {
                ...
                if (analyticsArr.length > 0) {
                    ...
                    analyticsArr = await updateAnalyticsExercises(analyticsArr, athlete)(dispatch, getState)
                }
