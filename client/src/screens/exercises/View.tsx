...
const Exercise = ({ route, navigation, pinExercises, exercisesStore, offline, dispatch, user, athleteProps }: Props) => {
        ...
        navigation.setOptions({
            headerRight: () => (
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                    <Pressable style={styles.actionContainer} onPress={onNavigateToAnalytics} hitSlop={5}>
                        <View style={styles.edit}>
                            <GraphSvg fillColor={BaseColors.black} />
                        </View>
                    </Pressable>
                    {athlete ?
                        <Pressable style={styles.actionContainer} onPress={onReportImage} hitSlop={5}>
                            <Pressable style={styles.edit} onPress={onReportImage}>
                                <ErrorSvg fillColor={BaseColors.black} />
                            </Pressable>
                        </Pressable>
                        : <Pressable style={styles.actionContainer} onPress={onNavigateToUpdate} hitSlop={5}>
                            <Pressable style={styles.edit} onPress={onNavigateToUpdate}>
                                <PencilSvg fillColor={BaseColors.black} />
                            </Pressable>
                        </Pressable>
                    }
                </View>
    ...
    useEffect(() => {
        exercise && setIsPin(pinExercises.find(p => p.exerciseUid === exercise._id) ? true : false)
    ...
    const onNavigateToAnalytics = () => {
        if (!exercise) return;
        if (athlete) {
            navigation.navigate(NetworkStackScreens.AthleteAnalytics, { exerciseUid: exercise._id })
        } else {
            if (route.params && route.params.programStack) {
                navigation.navigate(ProgramStackScreens.ProgramExerciseAnalytics, { exerciseUid: exercise._id })
            } else {
                navigation.navigate(HomeStackScreens.ExerciseAnalytics, { exerciseUid: exercise._id })
            }
        }
    }
    ...
    return (
        <SafeAreaView style={{ flex: 1 }} edges={['left', 'right', 'bottom']}>
            <ScrollView style={[styles.container, { marginTop: headerHeight }]} contentContainerStyle={{ paddingBottom: StyleConstants.baseMargin }}>
                <PrimaryText styles={styles.header}>{exercise.name}</PrimaryText>
                {!!exercise.description && <SecondaryText styles={styles.des}>{exercise.description}</SecondaryText>}

                {
                    !athlete && <View style={styles.pinContainer}>
                        <Switch
                            onSwitch={() => onUpdatePinExercises(isPin ? false : true)}
                            active={isPin}
                            styles={{ marginRight: StyleConstants.smallMargin, top: -5 }}
                        />
                        <SecondaryText styles={styles.label}>{isPin ? 'Pinned' : 'Unpinned'}</SecondaryText>
                    </View>
                }
                ...
                <ScrollView
                    ...
                    contentContainerStyle={{
                        flexDirection: (exercise.localUrl || exercise.url) ? 'row' : 'row-reverse'
                    }}
                >
                ...
                >
                    ...
                    <View style={styles.itemContainer}>
                        ...
                        <SecondaryText styles={[styles.text, styles.textCap]} bold>{exercise.category}</SecondaryText>
                    </View>
                    ...
                    <View style={styles.itemContainer}>
                        ...
                        <SecondaryText styles={[styles.text, styles.textCap]} bold>{exercise.equipment}</SecondaryText>
                    </View>

                    <View style={styles.itemContainer}>
                        <View style={styles.svg}>
                            <ScaleSvg fillColor={BaseColors.primary} />
                        </View>
                        <SecondaryText styles={styles.label}>Measurement</SecondaryText>
                        <SecondaryText styles={[styles.text, styles.textCap]} bold>{exercise.measCat}</SecondaryText>
                    </View>
                    ...
                    <View style={styles.itemContainer}>
                        ...
                        <SecondaryText styles={[styles.text, styles.textCap]} bold>{exercise.measSubCat}</SecondaryText>
                    </View>
