...
import { DataProps, GroupedProps, Stats } from '../../components/data-overview/types';
...
const DataOverview = ({ healthData }: Props) => {
    ...
    const formatHealthData = useCallback(() => {
        ...
        const grouped: GroupedProps = _.reduce(healthData, (result, value) => {
        ...
        for (let key in grouped) {
            if (grouped.hasOwnProperty(key)) {
                let healthData = grouped[key];

                //sort by dates
                healthData.sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime())

                let disVals = healthData.map(d => d.distance);
                let durVals = healthData.map(d => d.duration);
                let calVals = healthData.map(d => d.calories);
                let dates = healthData.map(d => d.date);
    ...
    const renderItem = useCallback(({ item }: { item: DataProps }) => {
    ...
    return (
        <SafeAreaView style={styles.container} edges={['left', 'right', 'bottom']}>
            <View style={styles.headerContainer}>
                <Pressable onPress={() => setPicker(true)} style={styles.filterContainer}>
                    <SecondaryText styles={styles.filterText} bold>{capitalize(filter)}</SecondaryText>
            ...
            <FlatList
                ...
                keyExtractor={(item, index) => item.activityName ? item.activityName : index.toString()}
            />
