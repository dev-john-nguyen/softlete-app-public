...
import { WorkoutExerciseProps, WorkoutActionProps, RootWorkoutProps, WorkoutExercisesObjProps } from '../../services/workout/types';
...
const ExerciseRestructure = ({ route, navigation, viewWorkout, updateWorkoutExercises, setBanner }: Props) => {
    ...
    const initiateData = useCallback(() => {
        if (viewWorkout) {
            if (!viewWorkout.exercises || viewWorkout.exercises.length < 1) {
                setBanner(BannerTypes.warning, "Workout doesn't contain any exercises.")
                navigation.goBack()
                return;
            }
        ...
        <SafeAreaView style={{ flex: 1 }} edges={['bottom', 'left', 'right']}>
            <View style={[{ height: headerHeight }, styles.headerContainer]}>
                ...
                <Pressable style={styles.rightContainer} onPress={onSave} hitSlop={5}>
                    {
                        loading ?
                            <ActivityIndicator size='small' color={BaseColors.black} /> : <SaveSvg strokeColor={BaseColors.black} />
                    }
                </Pressable>
            </View>
            <ExerciseList
...
const styles = StyleSheet.create({
    headerContainer: {
        flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end', paddingBottom: 10
    },
    leftContainer: {

    },
    middleText: {
        fontSize: StyleConstants.smallerFont, color: BaseColors.black
    },
    rightContainer: {
        height: normalize.width(20), width: normalize.width(20), marginRight: StyleConstants.baseMargin
    }
})
...
export default connect(mapStateToProps, { updateWorkoutExercises, setBanner })(ExerciseRestructure);