...
const GoOnlineModal = ({ navigation, route, dispatch, goOnline }: Props) => {
    ...
    useLayoutEffect(() => {
        navigation.setOptions({
            headerLeft: () => !loading && <BackButton onPress={() => navigation.navigate(HomeStackScreens.Home)} />,
            ...
            .then(async (state) => {
                if (state.isConnected) {
                    try {
                        ...
                        console.log(err)
                        setText("Failed to save all changes. Please ensure you have a good internet connection.")
                    }
                }
                setLoading(false)
            })
    ...
    return (
        <SafeAreaView style={styles.container} edges={['left', 'right', 'bottom']}>
            <View style={styles.content}>
                ...
                {loading && <ActivityIndicator size='large' color={BaseColors.primary} />}
            </View>
        </SafeAreaView >
    )
}
...
const mapDispatchToProps = (dispatch: any) => {
    return {
        goOnline: async (callBack: (msg: string) => void) => dispatch(goOnline(callBack)),
        dispatch
    }
}
