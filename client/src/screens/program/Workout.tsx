...
const Workout = ({ route, navigation, workout, updateWorkoutStatus, targetProgram, dispatch, updateProgramWoHealthData }: Props) => {
    ...
    useLayoutEffect(() => {
        navigation.setOptions({
            headerRight: () => (workout?.status !== WorkoutStatus.inProgress) && !disableEdit() && (
                <Pressable style={styles.menu} onPress={() => navigation.navigate(ProgramStackScreens.ProgramWorkoutModal)}>
    ...
    const onUpdateWoHealthData = async (workoutUid: string, data: HealthDataProps) => {
        await updateProgramWoHealthData(workout.programTemplateUid, workoutUid, data).catch(err => console.log(err))
    }
