...
import { FriendProps, UserProps, FriendStatus, UserActionProps } from '../../services/user/types';
...
import { NotificationActionProps, NotificationProps } from '../../services/notifications/types';
...
const Notifications = ({ navigation, user, dispatch, athletes, sendFriendRequest, notifications, fetchNotifications, getFriends }: Props) => {
    ...
    const onFriendRequestResponse = (userUid: string, accept: boolean) => {
        sendFriendRequest(userUid, accept ? FriendStatus.accepted : FriendStatus.denied)
            .catch((err) => {
                console.log(err)
            })
    }
    ...
    const onNavToAthlete = (athlete?: AthleteProfileProps | string) => {
        ...
        navigation.navigate(NetworkStackScreens.AthleteDashboard, { athlete: athObj })
    }

    const renderFriendsItem = useCallback((item: FriendProps, index: number) => {
        return <FriendItem
            friend={item}
    ...
    const renderNotificationItem = useCallback(({ item, index }: { item: NotificationProps, index: number }) => {
        return <NotificationItem notification={item} key={item._id ? item._id : index} onPress={() => item.data && item.data.senderProps && onNavToAthlete(item.data.senderProps)} />
    }, [notifications])


    const renderItem = useCallback(({ item, index, section }: { item: any, index: number, section: { title: string, data: any[] } }) => {
